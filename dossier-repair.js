...
(() => {
  ...
  const showPlanetaryDirectory = (parts) => {
    ...
    if (!file) return;
    target.innerHTML = `<div class="occult-page"><h2 class="welcome">${file.title}</h2><p class="occult-intro">${file.intro}</p><table class="archive-table"><tr><th>SYMBOL</th><th>NAME</th><th>ASSOCIATIONS</th></tr>${file.rows.map(([symbol,name,meaning]) => `<tr><td style="font:26px Georgia;text-align:center">${symbol}</td><td><b>${name}</b></td><td>${meaning}</td></tr>`).join('')}</table><section class="occult-article"><h3>HISTORY &amp; DEVELOPMENT</h3><p>${file.history}</p></section>${file.timeline ? `<section class="occult-article"><h3>DATED SYMBOL RECORD</h3>${file.timeline}</section>` : ''}<section class="occult-sources"><h3>IMAGE &amp; READING NOTES</h3><p>The visual material above comes from historic astrological manuscript and planetary-symbol comparison imagery. This directory will expand with individual symbol records, manuscript examples, and cross-links to related occult traditions.</p></section><p class="occult-return"><a href="#occult-symbols/planetary-astrological-symbols">[ BACK TO PLANETARY DIRECTORY ]</a> &nbsp; <a href="#occult-symbols">[ SYMBOLS &amp; SYMBOLISM ]</a></p></div>`;
  };
