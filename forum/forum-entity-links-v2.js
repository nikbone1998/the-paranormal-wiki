(() => {
  'use strict';
  const CACHE_KEY='tpw:canonical-entity-index:v2';
  ...
  async function loadIndex(){
    ...
    entityIndexPromise=(async()=>{
      ...
      if(entries.length<MIN_TRUSTED_ENTRIES)throw new Error(`Only ${entries.length} canonical entity routes were verified.`);
  ...
  function exactMatch(value){
    ...
    return entityIndex?.find(e=>String(e.name).toLocaleLowerCase()===value||String(e.slug).toLocaleLowerCase()===value)||null;
  }
  ...
  function installPickerShell(){
    ...
    idField.before(wrap);
    idField.classList.add('entity-legacy-field');nameField.classList.add('entity-legacy-field');
  ...
  async function preparePicker(){
    ...
    status.textContent='Loading verified canonical archive entities…';
    try{
      ...
      status.textContent=`${entries.length} canonical archive entities verified. Choose an exact match to attach it.`;
      syncPickerSelection();
    }catch(error){
      ...
      status.textContent='Automatic canonical selector is unavailable. Manual fields remain visible; no route will be guessed.';
    }
  }
  ...
  function syncPickerSelection(){
    ...
    idInput.value=match.id||`slug:${match.slug}`;nameInput.value=match.name;
  ...
  async function upgradeThreadLinks(){
    ...
    try{
      const entries=await loadIndex();
      const byName=new Map(entries.map(e=>[String(e.name).toLocaleLowerCase(),e]));
      const byId=new Map(entries.filter(e=>e.id).map(e=>[String(e.id).toLocaleLowerCase(),e]));
      root.querySelectorAll('.entity-tag').forEach(tag=>{
        const raw=normalize(tag.textContent).replace(/^ARCHIVE LINK:\s*/i,'');
        const idFromTitle=(tag.getAttribute('title')||'').replace(/^Archive entity ID:\s*/i,'').trim();
        const match=byId.get(idFromTitle.toLocaleLowerCase())||byName.get(raw.toLocaleLowerCase());
        if(!match)return;
  ...
  const observer=new MutationObserver(()=>{installPickerShell();if(document.querySelector('#threadEntity .entity-tag'))upgradeThreadLinks()});
