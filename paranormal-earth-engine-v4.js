...
async function mount(root){
 ...
 try{
  ...
  const tileFS=`precision mediump float;uniform sampler2D tMap;uniform vec3 uLightDir;uniform float uOpacity;varying vec2 vUv;varying vec3 vN;void main(){float ndl=dot(normalize(vN),normalize(uLightDir)),gate=smoothstep(-.15,.16,ndl),diff=pow(max(ndl,0.0),.42);vec3 c=texture2D(tMap,vUv).rgb*(.05+gate*(.48+1.02*diff));gl_FragColor=vec4(c,uOpacity*gate);#include <tonemapping_fragment>#include <colorspace_fragment>}`;
  ...
  const api={version:VERSION,reset:resetView,detailView,flyTo(o={}){tEl=rad(clamp(Number(o.lat)||0,-84,84));tAz=wrap(rad(Number(o.lon)||0));if(Number.isFinite(o.altitude))tDist=capDist(1+Math.max(isFull()?120:250,o.altitude)/EARTH_RADIUS_KM);markMove();},focusRegion(o){this.flyTo(o);},getObservationState(){return {altitudeKm:Math.max(0,(dist-1)*EARTH_RADIUS_KM),lod:lodLevel,detail:lodState,center:vecLatLon(camera.position),activeTiles:[...cache.values()].filter(e=>e.state==='ready'&&e.opacity>.04).length,illumination:state.realtime?'real-time':'observation'};},destroy(){if(disposed)return;disposed=true;if(raf)cancelAnimationFrame(raf);for(const f of clean.splice(0))try{f()}catch{}document.body.classList.remove('ua-earth-lock');root.style.touchAction='pan-y';for(const e of cache.values())if(e.mesh){e.mesh.geometry.dispose();e.mesh.material.uniforms.tMap.value.dispose();e.mesh.material.dispose();}renderer?.dispose?.();mounted.delete(root);},addMarker(){return null;},removeMarker(){return false;},setMarkerLayer(){return true;},setOverlay(){return true;}};
  mounted.set(root,api);labels();updateFs();setReady();requestFrame();highGlobal();return api;
 }catch(err){setFallback(err);const api={version:VERSION,destroy(){disposed=true;},reset:resetView};mounted.set(root,api);return api;}
}
