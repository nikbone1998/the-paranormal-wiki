...
async function mount(root){
 ...
 let markerGroup=null,markerLabelGroup=null,markerGeometry=null,markerMaterials=null,markerItems=[],markerPayload=null,markerSelect=null;
...
#include <colorspace_fragment>
  ...
  markerGroup=new THREE.Group();markerGroup.name='source-grounded-dossier-markers';markerLabelGroup=new THREE.Group();markerLabelGroup.name='source-grounded-dossier-labels';root.dataset.earthMarkerMode='three-depth-tested';root.dataset.earthMarkerDepth='true';scene.add(markerGroup,markerLabelGroup);markerGeometry=new THREE.SphereGeometry(.026,10,10);markerMaterials={traditional:new THREE.MeshBasicMaterial({color:'#00ffff',depthTest:true,depthWrite:false,transparent:false,opacity:1}),historical:new THREE.MeshBasicMaterial({color:'#ffff00',depthTest:true,depthWrite:false,transparent:false,opacity:1}),modern:new THREE.MeshBasicMaterial({color:'#cc66ff',depthTest:true,depthWrite:false,transparent:false,opacity:1}),famous:new THREE.MeshBasicMaterial({color:'#ff3333',depthTest:true,depthWrite:false,transparent:false,opacity:1})};
...
#include <colorspace_fragment>
  ...
  function rebuildMarkers(){if(!markerGroup||!THREE||!markerPayload)return;for(const entry of markerItems){markerGroup.remove(entry.mesh,entry.hit);if(entry.label){markerLabelGroup.remove(entry.label);entry.hit.material.dispose();entry.label.material.map.dispose();entry.label.material.dispose();}else entry.hit.material.dispose();}markerItems=[];const source=markerPayload.items||[];for(const item of source){const key=String(item.point?.layer||'').toLowerCase(),layer=({reported:'modern',case:'famous',cases:'famous'}[key]||key),color=markerMaterials[layer]?.color?.getStyle?.()||'#cc66ff',mesh=new THREE.Mesh(markerGeometry,markerMaterials[layer]||markerMaterials.modern),hit=new THREE.Mesh(markerGeometry,new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthTest:true,depthWrite:false}));mesh.position.copy(latLonVec(THREE,Number(item.point.lat),Number(item.point.lon),1.018));hit.position.copy(mesh.position);mesh.userData.selection=hit.userData.selection=[item];mesh.frustumCulled=hit.frustumCulled=false;markerGroup.add(mesh,hit);markerItems.push({item,mesh,hit,label:null,baseLabel:item.entity.name,color});}markerSelect=markerPayload.onSelect||null;root.dataset.earthMarkerCount=String(markerItems.length);}
  function updateMarkers(){if(!markerGroup||!camera||!markerItems.length)return;camera.updateMatrixWorld(true);camera.updateProjectionMatrix();markerGroup.updateMatrixWorld(true);markerLabelGroup.updateMatrixWorld(true);const groups=[],used=new Set(),rect=canvas.getBoundingClientRect(),zoomLevel=clamp((dist-1)/1.9,0,1),threshold=10+zoomLevel*24,projected=[];for(let i=0;i<markerItems.length;i++){const entry=markerItems[i],normal=entry.mesh.position.clone().normalize(),toCamera=camera.position.clone().sub(entry.mesh.position).normalize(),front=normal.dot(toCamera)>.018;entry.mesh.visible=true;entry.hit.visible=front;if(entry.label)entry.label.visible=false;if(!front){projected[i]=null;continue;}const p=entry.mesh.position.clone().project(camera),view=entry.mesh.position.clone().applyMatrix4(camera.matrixWorldInverse),depth=Math.max(.015,-view.z),pixelsPerUnit=rect.height/(2*depth*Math.tan(rad(camera.fov)/2)),dotScale=clamp(7/(pixelsPerUnit*.026),.001,1.2);entry.mesh.scale.setScalar(dotScale);entry.hit.scale.setScalar(Math.max(.05,dotScale*2.7));projected[i]={x:(p.x+1)*.5*rect.width,y:(1-p.y)*.5*rect.height,depth};}for(let i=0;i<markerItems.length;i++){if(!projected[i]||used.has(i))continue;const members=[];for(let j=i;j<markerItems.length;j++){if(!projected[j]||used.has(j))continue;if(Math.hypot(projected[j].x-projected[i].x,projected[j].y-projected[i].y)<threshold)members.push(j);}members.forEach(j=>used.add(j));groups.push(members);}const closeEnough=dist<1.72;for(const members of groups){const lead=markerItems[members[0]],cluster=members.length>1;lead.mesh.visible=lead.hit.visible=true;lead.mesh.scale.setScalar(clamp((cluster?18:7)/(pixelsPerUnitFor(lead.mesh,rect)*.026),.001,1.2));lead.hit.scale.setScalar(Math.max(.05,lead.mesh.scale.x*2.7));lead.mesh.userData.selection=lead.hit.userData.selection=members.map(i=>markerItems[i].item);const showLabel=cluster||(closeEnough&&members.length===1);if(showLabel){const label=ensureLabel(lead,cluster?`${members.length} LOCATIONS`:lead.baseLabel,lead.color);label.position.copy(lead.mesh.position.clone().normalize().multiplyScalar(cluster?1.055:1.045));label.visible=true;const labelDepth=Math.max(.015,-label.position.clone().applyMatrix4(camera.matrixWorldInverse).z),labelPpu=rect.height/(2*labelDepth*Math.tan(rad(camera.fov)/2)),labelH=clamp(16/labelPpu,.0015,.12);label.scale.set(labelH*8,labelH,1);}for(let k=1;k<members.length;k++){const entry=markerItems[members[k]];entry.mesh.visible=false;entry.hit.visible=false;if(entry.label)entry.label.visible=false;}}}
  function pixelsPerUnitFor(mesh,rect){const depth=Math.max(.015,-mesh.position.clone().applyMatrix4(camera.matrixWorldInverse).z);return rect.height/(2*depth*Math.tan(rad(camera.fov)/2));}
