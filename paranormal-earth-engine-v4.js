...
#include <colorspace_fragment>
  ...
  function updateMarkers(){if(!markerGroup||!camera||!markerItems.length)return;camera.updateMatrixWorld(true);camera.updateProjectionMatrix();const groups=[],used=new Set(),rect=canvas.getBoundingClientRect(),threshold=Math.max(14,Math.min(28,rect.width/58)),projected=[];for(let i=0;i<markerItems.length;i++){const entry=markerItems[i],normal=entry.mesh.position.clone().normalize(),toCamera=camera.position.clone().sub(entry.mesh.position).normalize(),front=normal.dot(toCamera)>.018;entry.mesh.visible=entry.hit.visible=front;if(entry.label)entry.label.visible=false;if(!front){projected[i]=null;continue;}const p=entry.mesh.position.clone().project(camera),view=entry.mesh.position.clone().applyMatrix4(camera.matrixWorldInverse),depth=Math.max(.015,-view.z),pixelsPerUnit=rect.height/(2*depth*Math.tan(rad(camera.fov)/2)),dotScale=clamp(7/(pixelsPerUnit*.026),.004,1.2);entry.mesh.scale.setScalar(dotScale);entry.hit.scale.setScalar(dotScale*2.7);projected[i]={x:(p.x+1)*.5*rect.width,y:(1-p.y)*.5*rect.height,depth};}for(let i=0;i<markerItems.length;i++){if(!projected[i]||used.has(i))continue;const members=[];for(let j=i;j<markerItems.length;j++){if(!projected[j]||used.has(j))continue;if(Math.hypot(projected[j].x-projected[i].x,projected[j].y-projected[i].y)<threshold)members.push(j);}members.forEach(j=>used.add(j));groups.push(members);}const closeEnough=dist<1.72;for(const members of groups){const lead=markerItems[members[0]],cluster=members.length>1;lead.mesh.visible=lead.hit.visible=true;lead.mesh.scale.setScalar(clamp((cluster?18:7)/(pixelsPerUnitFor(lead.mesh,rect)*.026),.004,1.2));lead.hit.scale.setScalar(lead.mesh.scale.x*2.7);lead.mesh.userData.selection=lead.hit.userData.selection=members.map(i=>markerItems[i].item);const showLabel=cluster||(closeEnough&&members.length===1);if(showLabel){const label=ensureLabel(lead,cluster?`${members.length} LOCATIONS`:lead.baseLabel,lead.color);label.position.copy(lead.mesh.position.clone().normalize().multiplyScalar(cluster?1.055:1.045));label.visible=true;const labelDepth=Math.max(.015,-label.position.clone().applyMatrix4(camera.matrixWorldInverse).z),labelPpu=rect.height/(2*labelDepth*Math.tan(rad(camera.fov)/2)),labelH=clamp(16/labelPpu,.0015,.12);label.scale.set(labelH*8,labelH,1);}for(let k=1;k<members.length;k++){const entry=markerItems[members[k]];entry.mesh.visible=entry.hit.visible=false;if(entry.label)entry.label.visible=false;}}}
