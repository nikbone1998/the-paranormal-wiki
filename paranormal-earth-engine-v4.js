...
function profile(){
 ...
 if(save)return {name:'DATA SAVER',tier:'SAVER',segments:64,dpr:1.2,maxLod:0,radius:0,cache:0,concurrency:0,terrain:false,globalWidth:16384};
 if(mobile)return {name:'ADAPTIVE / MOBILE',tier:'MOBILE',segments:128,dpr:1.65,maxLod:0,radius:0,cache:0,concurrency:0,terrain:false,globalWidth:16384};
 if(mem>=8&&cores>=6)return {name:'HIGH / DESKTOP',tier:'HIGH',segments:256,dpr:2,maxLod:0,radius:0,cache:0,concurrency:0,terrain:false,globalWidth:16384};
 return {name:'ADAPTIVE / DESKTOP',tier:'ADAPTIVE',segments:192,dpr:1.8,maxLod:0,radius:0,cache:0,concurrency:0,terrain:false,globalWidth:16384};
}
...
async function mount(root){
 ...
 let markerGroup=null,markerLabelGroup=null,markerGeometry=null,markerMaterials=null,markerItems=[],markerPayload=null,markerSelect=null,markerClusterSignature='',markerClusterGroups=[];
 ...
 const pointers=new Map();let dragId=null,lastX=0,lastY=0,lastPinch=0,lastTouchPinch=0,clickStartX=0,clickStartY=0;
 ...
 on(root,'pointerdown',e=>{pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size===1){clickStartX=e.clientX;clickStartY=e.clientY;}root.setPointerCapture?.(e.pointerId);markMove();if(pointers.size===1){dragId=e.pointerId;lastX=e.clientX;lastY=e.clientY;root.classList.add('ua-dragging');}else{dragId=null;lastPinch=pinch();}},{passive:true});
 ...
 const pickMarker=e=>{if(!renderer||!camera||!markerItems.length||e.target.closest?.('.ua-globe-controls,.ua-observation-panel,.ua-entity-layer'))return;const r=canvas.getBoundingClientRect();if(!r.width||!r.height)return;const ndc=new THREE.Vector2(((e.clientX-r.left)/r.width)*2-1,-((e.clientY-r.top)/r.height)*2+1),ray=new THREE.Raycaster();ray.setFromCamera(ndc,camera);const hits=ray.intersectObjects(markerItems.map(x=>x.hit).filter(x=>x.visible),false);if(hits[0]?.object.userData.selection)markerSelect?.(hits[0].object.userData.selection);};
 const end=e=>{const wasClick=Math.hypot(e.clientX-clickStartX,e.clientY-clickStartY)<8;pointers.delete(e.pointerId);if(e.pointerId===dragId)dragId=null;if(pointers.size<2)lastPinch=0;if(!pointers.size)root.classList.remove('ua-dragging');markMove();if(wasClick)pickMarker(e);};on(root,'pointerup',end);on(root,'pointercancel',end);on(root,'lostpointercapture',end);
...
#include <colorspace_fragment>
  ...
  updateLod=()=>{lodLevel=0;lodState='PRE-RENDERED GLOBAL';};
  animateTiles=()=>{};
  function labelSprite(text,color){const c=document.createElement('canvas');c.width=512;c.height=64;const x=c.getContext('2d');x.font='bold 25px Arial';x.textBaseline='middle';const w=Math.min(500,Math.max(80,x.measureText(text).width+26));x.fillStyle='rgba(0,0,8,.82)';x.strokeStyle=color;x.lineWidth=2;x.beginPath();x.roundRect(2,5,w-4,54,9);x.fill();x.stroke();x.fillStyle='#fff';x.fillText(text,14,32);const t=new THREE.CanvasTexture(c);t.colorSpace=THREE.SRGBColorSpace;t.needsUpdate=true;const m=new THREE.SpriteMaterial({map:t,transparent:true,depthTest:true,depthWrite:false});const s=new THREE.Sprite(m);s.userData={canvas:c,text,color};return s;}
  function setLabel(sprite,text,color){if(sprite.userData.text===text&&sprite.userData.color===color)return;const c=sprite.userData.canvas,x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);x.font='bold 25px Arial';x.textBaseline='middle';const w=Math.min(500,Math.max(80,x.measureText(text).width+26));x.fillStyle='rgba(0,0,8,.82)';x.strokeStyle=color;x.lineWidth=2;x.beginPath();x.roundRect(2,5,w-4,54,9);x.fill();x.stroke();x.fillStyle='#fff';x.fillText(text,14,32);sprite.material.map.needsUpdate=true;sprite.userData.text=text;sprite.userData.color=color;}
  function ensureLabel(entry,text,color){if(!entry.label){entry.label=labelSprite(text,color);entry.label.frustumCulled=false;markerLabelGroup.add(entry.label);}setLabel(entry.label,text,color);return entry.label;}
  function rebuildMarkers(){if(!markerGroup||!THREE||!markerPayload)return;for(const entry of markerItems){markerGroup.remove(entry.mesh,entry.hit);if(entry.label){markerLabelGroup.remove(entry.label);entry.hit.material.dispose();entry.label.material.map.dispose();entry.label.material.dispose();}else entry.hit.material.dispose();}markerItems=[];markerClusterSignature='';markerClusterGroups=[];const source=markerPayload.items||[];for(const item of source){const key=String(item.point?.layer||'').toLowerCase(),layer=({reported:'modern',case:'famous',cases:'famous'}[key]||key),color=markerMaterials[layer]?.color?.getStyle?.()||'#a88bd8',mesh=new THREE.Mesh(markerGeometry,markerMaterials[layer]||markerMaterials.modern),hit=new THREE.Mesh(markerGeometry,new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthTest:true,depthWrite:false}));mesh.position.copy(latLonVec(THREE,Number(item.point.lat),Number(item.point.lon),1.018));hit.position.copy(mesh.position);mesh.userData.selection=hit.userData.selection=[item];mesh.frustumCulled=hit.frustumCulled=false;markerGroup.add(mesh,hit);markerItems.push({item,mesh,hit,label:null,baseLabel:item.entity.name,color});}markerSelect=markerPayload.onSelect||null;root.dataset.earthMarkerCount=String(markerItems.length);}
  function updateMarkers(){if(!markerGroup||!camera||!markerItems.length)return;camera.updateMatrixWorld(true);camera.updateProjectionMatrix();markerGroup.updateMatrixWorld(true);markerLabelGroup.updateMatrixWorld(true);const used=new Set(),rect=canvas.getBoundingClientRect(),zoomLevel=clamp((dist-1)/1.9,0,1),threshold=10+zoomLevel*24,clusterSignature=`${Math.round(az*360/Math.PI)}:${Math.round(el*360/Math.PI)}:${Math.round(dist*100)}:${Math.round(rect.width)}x${Math.round(rect.height)}:${Math.round(zoomLevel*20)}`,cameraDirection=camera.position.clone().normalize(),projected=[];for(let i=0;i<markerItems.length;i++){const entry=markerItems[i],normal=entry.mesh.position.clone().normalize(),facing=normal.dot(cameraDirection),front=facing>.02||(entry.hit.visible&&facing>-.035);entry.hit.visible=front;if(entry.label)entry.label.visible=false;if(!front){projected[i]=null;continue;}const p=entry.mesh.position.clone().project(camera),view=entry.mesh.position.clone().applyMatrix4(camera.matrixWorldInverse),depth=Math.max(.015,-view.z),pixelsPerUnit=rect.height/(2*depth*Math.tan(rad(camera.fov)/2)),dotScale=clamp(7/(pixelsPerUnit*.026),.001,1.2);entry.mesh.scale.setScalar(dotScale);entry.hit.scale.setScalar(Math.max(.05,dotScale*2.7));projected[i]={x:(p.x+1)*.5*rect.width,y:(1-p.y)*.5*rect.height,depth};}if(!projected.some(Boolean))return;for(const entry of markerItems)entry.mesh.visible=true;let groups=markerClusterGroups;if(markerClusterSignature!==clusterSignature){groups=[];for(let i=0;i<markerItems.length;i++){if(!projected[i]||used.has(i))continue;const members=[];for(let j=i;j<markerItems.length;j++){if(!projected[j]||used.has(j))continue;if(Math.hypot(projected[j].x-projected[i].x,projected[j].y-projected[i].y)<threshold)members.push(j);}members.forEach(j=>used.add(j));groups.push(members);}markerClusterGroups=groups;markerClusterSignature=clusterSignature;}const closeEnough=dist<1.72;for(const members of groups){const valid=members.filter(i=>projected[i]);if(!valid.length)continue;const lead=markerItems[valid[0]],cluster=valid.length>1;lead.mesh.visible=lead.hit.visible=true;lead.mesh.scale.setScalar(clamp((cluster?18:7)/(pixelsPerUnitFor(lead.mesh,rect)*.026),.001,1.2));lead.hit.scale.setScalar(Math.max(.05,lead.mesh.scale.x*2.7));lead.mesh.userData.selection=lead.hit.userData.selection=valid.map(i=>markerItems[i].item);const showLabel=cluster||(closeEnough&&valid.length===1);if(showLabel){const label=ensureLabel(lead,cluster?`${valid.length} LOCATIONS`:lead.baseLabel,lead.color);label.position.copy(lead.mesh.position.clone().normalize().multiplyScalar(cluster?1.055:1.045));label.visible=true;const labelDepth=Math.max(.015,-label.position.clone().applyMatrix4(camera.matrixWorldInverse).z),labelPpu=rect.height/(2*labelDepth*Math.tan(rad(camera.fov)/2)),labelH=clamp(16/labelPpu,.0015,.12);label.scale.set(labelH*8,labelH,1);}for(let k=1;k<valid.length;k++){const entry=markerItems[valid[k]];entry.mesh.visible=false;entry.hit.visible=false;if(entry.label)entry.label.visible=false;}}}
  /* Stable replacement: close views reveal the exact front-facing records instead
     of inheriting a stale cluster lead from the previous camera state. */
  function updateMarkersStable(){
   if(!markerGroup||!camera||!markerItems.length)return;
  ...
  async function highGlobal(){if(navigator.connection?.saveData)return;await new Promise(r=>setTimeout(r,650));if(disposed)return;try{const im=await image(ASSETS.nasaGlobal),max=renderer.capabilities.maxTextureSize||4096,w=Math.min(p.globalWidth,max),h=Math.round(w/2);let tex;if(im.width<=w){tex=new THREE.Texture(im);}else{const c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.drawImage(im,0,0,w,h);tex=new THREE.CanvasTexture(c);}tex.needsUpdate=true;tex.colorSpace=THREE.SRGBColorSpace;tex.anisotropy=Math.min(maxAniso,12);tex.generateMipmaps=true;tex.minFilter=THREE.LinearMipmapLinearFilter;const old=surfaceU.tDay.value;surfaceU.tDay.value=tex;globalLabel=`${Math.round(w/1024)}K PRE-RENDERED GLOBAL`;old?.dispose?.();}catch(err){console.warn('[PARANORMAL WIKI EARTH] pre-rendered global surface unavailable',err?.message||err);}}
  ...
  const api={version:VERSION,reset:resetView,detailView,flyTo(o={}){tEl=rad(clamp(Number(o.lat)||0,-84,84));tAz=wrap(rad(Number(o.lon)||0));if(Number.isFinite(o.altitude))tDist=capDist(1+Math.max(isFull()?120:250,o.altitude)/EARTH_RADIUS_KM);markMove();},focusRegion(o){this.flyTo(o);},setMarkers(items,onSelect){markerPayload={items:Array.isArray(items)?items:[],onSelect};rebuildMarkers();requestFrame();return true;},getObservationState(){return {altitudeKm:Math.max(0,(dist-1)*EARTH_RADIUS_KM),lod:lodLevel,detail:lodState,center:vecLatLon(camera.position),activeTiles:[...cache.values()].filter(e=>e.state==='ready'&&e.opacity>.04).length,illumination:state.realtime?'real-time':'observation'};},addOverlay(draw){if(typeof draw!=='function')return()=>{};overlays.add(draw);requestFrame();return()=>overlays.delete(draw);},destroy(){if(disposed)return;disposed=true;if(raf)cancelAnimationFrame(raf);overlays.clear();for(const f of clean.splice(0))try{f()}catch{}for(const entry of markerItems){markerGroup?.remove(entry.mesh,entry.hit);if(entry.label){markerLabelGroup?.remove(entry.label);entry.hit.material.dispose();entry.label.material.map.dispose();entry.label.material.dispose();}else entry.hit.material.dispose();}markerMaterials&&Object.values(markerMaterials).forEach(m=>m.dispose());markerGeometry?.dispose();document.body.classList.remove('ua-earth-lock');root.style.touchAction='pan-y';for(const e of cache.values())if(e.mesh){e.mesh.geometry.dispose();e.mesh.material.uniforms.tMap.value.dispose();e.mesh.material.dispose();}renderer?.dispose?.();mounted.delete(root);},addMarker(){return null;},removeMarker(){return false;},setMarkerLayer(){return true;},setOverlay(draw){return this.addOverlay(draw);}};
  mounted.set(root,api);labels();updateFs();await highGlobal();if(disposed)return api;setReady();requestFrame();return api;
 }catch(err){setFallback(err);const api={version:VERSION,destroy(){disposed=true;},reset:resetView,flyTo(){},focusRegion(){},setMarkers(){return false;},getObservationState(){return {altitudeKm:12000,lod:0,detail:'FALLBACK',center:{lat:13,lon:25},activeTiles:0,illumination:'observation'};}};mounted.set(root,api);return api;}
}
