...
async function mount(root){
 ...
 let markerGroup=null,markerGeometry=null,markerMaterials=null,markerItems=[],markerPayload=null,markerSelect=null;
...
#include <colorspace_fragment>
  ...
  function rebuildMarkers(){if(!markerGroup||!THREE||!markerPayload)return;for(const entry of markerItems)markerGroup.remove(entry.mesh);markerItems=[];const source=markerPayload.items||[];for(const item of source){const key=String(item.point?.layer||'').toLowerCase(),layer=({reported:'modern',case:'famous',cases:'famous'}[key]||key),mesh=new THREE.Mesh(markerGeometry,markerMaterials[layer]||markerMaterials.modern);mesh.position.copy(latLonVec(THREE,Number(item.point.lat),Number(item.point.lon),1.018));mesh.userData.selection=[item];markerGroup.add(mesh);markerItems.push({item,mesh});}markerSelect=markerPayload.onSelect||null;root.dataset.earthMarkerCount=String(markerItems.length);}
  function updateMarkers(){if(!markerGroup||!camera||!markerItems.length)return;camera.updateMatrixWorld(true);const groups=[],used=new Set(),rect=canvas.getBoundingClientRect(),threshold=Math.max(18,Math.min(34,rect.width/42));const projected=[];for(let i=0;i<markerItems.length;i++){const {mesh}=markerItems[i],normal=mesh.position.clone().normalize(),toCamera=camera.position.clone().sub(mesh.position).normalize(),front=normal.dot(toCamera)>.018;mesh.visible=front;mesh.scale.setScalar(1);if(!front){projected[i]=null;continue;}const p=mesh.position.clone().project(camera);projected[i]={x:(p.x+1)*.5*rect.width,y:(1-p.y)*.5*rect.height};}for(let i=0;i<markerItems.length;i++){if(!projected[i]||used.has(i))continue;const members=[];for(let j=i;j<markerItems.length;j++){if(!projected[j]||used.has(j))continue;if(Math.hypot(projected[j].x-projected[i].x,projected[j].y-projected[i].y)<threshold)members.push(j);}members.forEach(j=>used.add(j));groups.push(members);}for(const members of groups){const lead=markerItems[members[0]];lead.mesh.visible=true;lead.mesh.scale.setScalar(members.length>1?1.8:1);lead.mesh.userData.selection=members.map(i=>markerItems[i].item);for(let k=1;k<members.length;k++)markerItems[members[k]].mesh.visible=false;}}
