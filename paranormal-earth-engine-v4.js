...
#include <colorspace_fragment>
  ...
  const api={version:VERSION,reset:resetView,detailView,flyTo(o={}){tEl=rad(clamp(Number(o.lat)||0,-84,84));tAz=wrap(rad(Number(o.lon)||0));if(Number.isFinite(o.altitude))tDist=capDist(1+Math.max(isFull()?120:250,o.altitude)/EARTH_RADIUS_KM);markMove();},focusRegion(o){this.flyTo(o);},getObservationState(){const c=camera.position.clone().normalize(),f={x:-c.x,y:-c.y,z:-c.z},up0={x:0,y:1,z:0},ud=f.x*up0.x+f.y*up0.y+f.z*up0.z,upRaw={x:up0.x-f.x*ud,y:up0.y-f.y*ud,z:up0.z-f.z*ud},ul=Math.hypot(upRaw.x,upRaw.y,upRaw.z)||1,up={x:upRaw.x/ul,y:upRaw.y/ul,z:upRaw.z/ul},right={x:f.y*up.z-f.z*up.y,y:f.z*up.x-f.x*up.z,z:f.x*up.y-f.y*up.x};return {altitudeKm:Math.max(0,(dist-1)*EARTH_RADIUS_KM),lod:lodLevel,detail:lodState,center:vecLatLon(camera.position),projection:{forward:f,right,up,focal:1/Math.tan(rad(camera.fov)/2),aspect:camera.aspect},activeTiles:[...cache.values()].filter(e=>e.state==='ready'&&e.opacity>.04).length,illumination:state.realtime?'real-time':'observation'};},addOverlay(draw){if(typeof draw!=='function')return()=>{};overlays.add(draw);requestFrame();return()=>overlays.delete(draw);},destroy(){if(disposed)return;disposed=true;if(raf)cancelAnimationFrame(raf);overlays.clear();for(const f of clean.splice(0))try{f()}catch{}document.body.classList.remove('ua-earth-lock');root.style.touchAction='pan-y';for(const e of cache.values())if(e.mesh){e.mesh.geometry.dispose();e.mesh.material.uniforms.tMap.value.dispose();e.mesh.material.dispose();}renderer?.dispose?.();mounted.delete(root);},addMarker(){return null;},removeMarker(){return false;},setMarkerLayer(){return true;},setOverlay(draw){return this.addOverlay(draw);}};
  mounted.set(root,api);labels();updateFs();setReady();requestFrame();highGlobal();return api;
 }catch(err){setFallback(err);const api={version:VERSION,destroy(){disposed=true;},reset:resetView,flyTo(){},focusRegion(){},getObservationState(){return {altitudeKm:12000,lod:0,detail:'FALLBACK',center:{lat:13,lon:25},activeTiles:0,illumination:'observation'};}};mounted.set(root,api);return api;}
}
